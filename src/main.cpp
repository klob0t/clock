...
unsigned long lastTrainCheck = 0;
char trainTextBuffer[80];
TrainEntry nextTrain;
...
void loop()
{
  ...
  if (nowMs - lastTrainCheck > TRAIN_CHECK_INTERVAL)
  {
    getTrainSchedule();
    lastTrainCheck = nowMs;
  }
...
void getTrainSchedule()
{
  ...
  const String destFilters[] = {TRAIN_DEST_1, TRAIN_DEST_2};
  TrainEntry entry = fetchNextTrain(TRAIN_STATION_ID, TRAIN_ORIGIN_ID, destFilters, 2);
  nextTrain = entry;
}
...
void updatePageLogic(unsigned long nowMs)
{
  ...
  case PAGE_TRAIN:
    if (duration > TRAIN_PAGE_DURATION_MS)
    {
  ...
  if (triggerZoneSetup)
  {
    ...
    else if (currentPage == PAGE_TRAIN)
    {
      P.setZone(0, 0, 3);
      P.setZone(1, 0, 0);
      P.setZone(2, 0, 0);
      ...
      if (nextTrain.valid)
      {
        ...
        unsigned long diff = (nextTrain.departEpoch > nowEpoch) ? (nextTrain.departEpoch - nowEpoch) : 0;
        unsigned long mins = diff / 60UL;
        unsigned long hrs = mins / 60UL;
        mins = mins % 60UL;
        if (hrs > 0)
          snprintf(trainTextBuffer, sizeof(trainTextBuffer), "%s-%s %luh%02lum", nextTrain.origin, nextTrain.destination, hrs, mins);
        else
          snprintf(trainTextBuffer, sizeof(trainTextBuffer), "%s-%s %lum", nextTrain.origin, nextTrain.destination, mins);
      }
      else
      {
        snprintf(trainTextBuffer, sizeof(trainTextBuffer), "No trains");
      }

      P.displayZoneText(0, trainTextBuffer, PA_LEFT, 40, 0, PA_SCROLL_LEFT, PA_SCROLL_LEFT);
      P.displayZoneText(1, "", PA_LEFT, 0, 0, PA_PRINT, PA_NO_EFFECT);
      P.displayZoneText(2, "", PA_LEFT, 0, 0, PA_PRINT, PA_NO_EFFECT);
    }
...
{
  if (target != PAGE_CLOCK && target != PAGE_WEATHER && target != PAGE_SUN && target != PAGE_TRAIN && target != PAGE_SCOPE)
    return;
  ...
  if (withTransition)
  {
    ...
    else if (target == PAGE_TRAIN)
      desired = TRANS_TO_TRAIN;
    else
...
void handleControlPackets(unsigned long nowMs)
{
  ...
  while (packetSize > 0)
  {
    ...
    if (len > 0)
    {
      ...
      else if (code == CTRL_CODE_PAGE_TRAIN)
      {
        forcePage(PAGE_TRAIN, nowMs, true);
      }
...
void updateDisplay(unsigned long nowMs)
{
  ...
  if (currentPage == TRANS_TO_WEATHER || currentPage == TRANS_TO_CLOCK || currentPage == TRANS_TO_SUN || currentPage == TRANS_TO_TRAIN || currentPage == TRANS_TO_SCOPE)
  {
    drawGlitch();
    for (int i = 0; i < TOTAL_WIDTH; i++)
  ...
  {
    TrainRenderState trainState{};
    trainState.hasData = nextTrain.valid;
    trainState.nowEpoch = (unsigned long)time(nullptr);
    if (nextTrain.valid)
    {
      snprintf(trainState.origin, sizeof(trainState.origin), "%s", nextTrain.origin);
      snprintf(trainState.destination, sizeof(trainState.destination), "%s", nextTrain.destination);
      trainState.departEpoch = nextTrain.departEpoch;
    }
    drawTrain(trainState, getTrainIcon(), screenBuffer, TOTAL_WIDTH);
    for (int i = 32; i < 40; i++)
      mx->setColumn(i, screenBuffer[i]);
  }
  else if (currentPage == PAGE_SCOPE)
