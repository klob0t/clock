...
const int CLOUD_GAP = 6; // Space between clouds

// Calculate total length of the cloud train (Clouds + Gaps)
...
void drawToBuffer(int startCol, uint64_t image, int width, int yOffset)
{
  ...
  for (int col = 0; col < width; col++) {
    ...
    // Strict Boundary Check (Don't wrap individual characters, just clip them)
    if (targetCol >= 0 && targetCol < HUD_WIDTH) {
      hudBuffer[targetCol] |= (columnByte << yOffset);
    }
  }
}

void drawCloudsToBuffer() {
  if (getHour() >= 12) return; // AM Only

  for(int i=0; i<NUM_CLOUDS; i++) {
    // 1. Calculate the ideal position in the cycle
    int offset = i * (CLOUD_WIDTH + CLOUD_GAP);
    float absolutePos = cloudPos + offset;

    // 2. Modulo operator to keep it inside the cycle length (e.g. 0 to 70)
    // fmod handles floats. We add TOTAL_CYCLE_LEN to handle negative numbers safely.
    float modPos = fmod(absolutePos, TOTAL_CYCLE_LEN);
    if(modPos < 0) modPos += TOTAL_CYCLE_LEN;

    int drawX = (int)modPos;

    // 3. Draw the cloud
    // Shift: 5 (Moves cloud to rows 5, 6, 7, 8 - effectively bottom aligned)
    drawToBuffer(drawX, CLOUDS[i], CLOUD_WIDTH, 5);

    // 4. Draw Wrap-Around Ghost
    // If the cloud is at the far right of the cycle (e.g., pos 68), 
    // we must also draw it at (68 - 70 = -2) so it appears entering from the left.
    drawToBuffer(drawX - TOTAL_CYCLE_LEN, CLOUDS[i], CLOUD_WIDTH, 5);
  }
}
...
void updateHudFast(unsigned long nowMs)
{
  ...
  if (nowMs - lastCloudTick > CLOUD_DELAY) {
    ...
    // Keep cloudPos manageable, but wrap based on Cycle Length
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // Draw Text at yOffset = 0 (Top)
    drawToBuffer(cursor, imageToDraw, 3, 0);
